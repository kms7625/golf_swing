...
### 3단계: 신규 웹 프론트엔드 구축 — ✅ 완료 (2026-07-05)

**결과**: `server/`(FastAPI, 3개 파일) + `web/`(React+Vite+TS, 8개 컴포넌트) 구축 완료.
동일성 검증 게이트 통과(테스트 영상 3종, API vs `analyzer/` 직접호출 — score/frame_data
길이/척추각/X-Factor/페이즈 경계 일치). `tsc --noEmit` + `npm run build` 통과. dev 서버
정적 서빙·CORS·샘플 로드 확인 완료. i18n은 원래 4단계 예정이었으나 컴포넌트 작성과
동시에(2단계 지시대로 하드코딩 없이) 구현 완료 — 별도 4단계 불필요.
**미검증**: 브라우저 확장 미연결로 클릭 기반 E2E(실제 업로드→트리밍→분석→코칭 전체
플로우를 브라우저에서 시각적으로 확인)는 못 함 — 재개 시 최우선 확인 항목.
**배포는 미착수** — 완료 기준의 ①②③은 충족, 배포 게이트는 별도.

**범위 확정 (2026-07-05, 사용자 합의)**:
